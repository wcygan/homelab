...
class NodeMaintenanceManager {
  ...
  private async drainNode(options: MaintenanceOptions) {
    console.log(colors.blue(`🔧 Preparing to drain node ${this.nodeInfo!.name} (${this.nodeInfo!.ip})`));

    // Pre-flight checks
    if (!options.skipEtcdCheck && this.nodeInfo!.roles.includes("control-plane")) {
    ...
    if (!options.force && !this.dryRun) {
      const confirm = await Confirm.prompt({
        message: `Drain node ${this.nodeInfo!.name}? This will evict ${pods.length} pods.`,
        default: false,
      });
      if (!confirm) return;
    }
    ...
    if (!this.dryRun) {
      ...
      console.log(colors.gray(`Would run: kubectl cordon ${this.nodeInfo!.name}`));
    }
    ...
    const drainCmd = `kubectl drain ${this.nodeInfo!.name} --ignore-daemonsets --delete-emptydir-data --force --timeout=${timeout}s`;
  ...
  private async restoreNode(options: MaintenanceOptions) {
    console.log(colors.blue(`🔧 Preparing to restore node ${this.nodeInfo!.name}`));
    ...
    if (this.nodeInfo!.status !== "Ready") {
      console.log(colors.yellow(`⚠️  Node is not ready: ${this.nodeInfo!.status}`));
      if (!options.force) {
        const proceed = await Confirm.prompt("Node is not ready. Proceed anyway?");
        if (!proceed) return;
      }
    }
    ...
    if (!this.dryRun) {
      ...
      console.log(colors.gray(`Would run: kubectl uncordon ${this.nodeInfo!.name}`));
      console.log(colors.green("✅ Dry run completed - no changes made"));
    }
  }
  ...
  private async showStatus() {
    ...
    console.log(`Status: ${this.nodeInfo!.status === "Ready" ? colors.green(this.nodeInfo!.status) : colors.red(this.nodeInfo!.status)}`);
    console.log(`Schedulable: ${this.nodeInfo!.schedulable ? colors.green("Yes") : colors.yellow("No (Cordoned)")}`);
    console.log(`Roles: ${this.nodeInfo!.roles.join(", ") || "worker"}`);
    
    // Resource usage
    try {
      const metrics = await $`kubectl top node ${this.nodeInfo!.name} --no-headers`.text();
      const [, cpu, cpuPercent, memory, memoryPercent] = metrics.trim().split(/\s+/);
      console.log(`\nResources:`);
  ...
  private async checkEtcdQuorum() {
    ...
    try {
      ...
      const healthyCount = etcdPods.items.filter((pod: any) => 
        ...
        pod.status.conditions.find((c: any) => c.type === "Ready" && c.status === "True")
      ).length;
      
      const totalCount = etcdPods.items.length;
      
      if (healthyCount < totalCount) {
        console.log(colors.red(`❌ Etcd quorum at risk: ${healthyCount}/${totalCount} healthy`));
        if (!this.dryRun) {
          throw new Error("Cannot proceed with degraded etcd cluster");
        }
      } else {
        console.log(colors.green(`✅ Etcd healthy: ${healthyCount}/${totalCount} members`));
      }
    } catch (error) {
  ...
  private async checkClusterHealth() {
    ...
    const nodes = await $`kubectl get nodes -o json`.json();
    const readyNodes = nodes.items.filter((n: any) => 
      n.status.conditions.find((c: any) => c.type === "Ready" && c.status === "True")
    ).length;
    
    const cordonedNodes = nodes.items.filter((n: any) => n.spec.unschedulable).length;
    
    console.log(`  Nodes: ${readyNodes}/${nodes.items.length} ready`);
    if (cordonedNodes > 0) {
      console.log(colors.yellow(`  ⚠️  ${cordonedNodes} nodes already cordoned`));
    }
  }
  ...
  private async checkWorkloadDistribution() {
    ...
    const avgPodsPerNode = otherNodes.reduce((sum, [, count]) => sum + count, 0) / otherNodes.length;
    ...
    console.log(`  Expected increase: +${expectedLoadIncrease.toFixed(1)} pods per node`);
  }

  private async getPodsOnNode(): Promise<PodInfo[]> {
    const result = await $`kubectl get pods --all-namespaces --field-selector spec.nodeName=${this.nodeInfo!.name} -o json`.json();
    
    return result.items
      .filter((pod: any) => !pod.metadata.ownerReferences?.some((ref: any) => ref.kind === "DaemonSet"))
      .map((pod: any) => ({
  ...
  private async verifyNodeEmpty() {
    const pods = await this.getPodsOnNode();
    if (pods.length > 0) {
      console.log(colors.yellow(`⚠️  ${pods.length} pods still on node (excluding DaemonSets)`));
    } else {
  ...
  private async showTalosInfo() {
    try {
      ...
      // Get Talos version
      const versionOutput = await $`talosctl version --nodes ${this.nodeInfo!.ip} --short`.text();
      const version = versionOutput.split('\n')[1]?.replace('Server:', '').trim() || "unknown";
      console.log(`  Version: ${version}`);
      
      // Get services status
      const services = await $`talosctl services -n ${this.nodeInfo!.ip}`.text();
...
if (import.meta.main) {
  await new Command()
    ...
    .option("-n, --node <node:string>", "Node name or IP address", { required: true })
    .option("-a, --action <action:string>", "Action to perform", { 
      required: true,
      enum: ["drain", "restore", "status"]
    })
    .option("--dry-run", "Simulate actions without making changes", { default: false })
    .option("-f, --force", "Skip confirmation prompts")
    .option("-t, --timeout <seconds:number>", "Drain timeout in seconds", { default: 300 })
    .option("--skip-etcd-check", "Skip etcd quorum verification (dangerous!)")
    .example("Check status", "node-maintenance.ts -n k8s-1 -a status")
    .example("Drain node (dry run)", "node-maintenance.ts -n k8s-1 -a drain --dry-run")
