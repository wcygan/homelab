// Wrapper for the existing flux config checker to make it importable
...
export class FluxConfigChecker {
  ...
  private createMonitoringResult(): MonitoringResult {
    ...
    return {
      ...
      details: [
        `Comprehensive checks: infinite-retries, health-checks, wait-config, timeout, sourceref-namespace, resource-constraints, interval-analysis`,
        ...this.issues.map(i => `${i.severity}: ${i.file} - ${i.message}`)
      ],
